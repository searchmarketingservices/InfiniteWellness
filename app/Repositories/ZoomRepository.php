...
class ZoomRepository
{
    ...
    {
        static $createdByName = '';
        if ($createdBy != null) {
            //            $createdByName = User::whereId($createdBy)->first()->roles->pluck('name')->first();
            $createdByName = User::whereId($createdBy)->first()->getRoleNames()->first();
        }
    ...
    public function generateZoomToken($data = [])
    {
        ...
        if ((isset($data['createdByName']) && $data['createdByName'] == 'Doctor')) {
